...
class AssetPathMacro extends MacroGenerator {
  ...
  Future<void> onGenerate(MacroState state) async {
    ...
    final className = config.generatedClassName.isNotEmpty ? config.generatedClassName : 'AssetPaths';
...
class AssetPathConfig {
  const AssetPathConfig({
    this.generatedClassName = 'AssetPaths',
    this.syncList = true,
    this.recursive = true,
    this.extension = '*',
  ...
  @override
  String toString() {
    return 'AssetPathConfig{generatedClassName: $generatedClassName, syncList: $syncList, recursive: $recursive, extension: $extension, rename: $rename, replacements: $replacements}';
  }
}
