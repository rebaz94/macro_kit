...
class AssetPathMacro extends MacroGenerator {
  ...
  Future<void> onAsset(MacroState state, MacroAssetDeclaration asset) async {
    ...
    var rename = config.rename ?? FieldRename.camelCase;
    ...
    if (config.syncList ?? true) {
      final entities = dir.listSync(recursive: config.recursive ?? true, followLinks: false);
      _processEntities(entities, absoluteBasePath, rename, folderStructure, subfolders);
    } else {
      final entities = await dir.list(recursive: config.recursive ?? true, followLinks: false).toList();
      _processEntities(entities, absoluteBasePath, rename, folderStructure, subfolders);
    }
  ...
  void _processEntities(
    ...
    for (final entity in entities) {
      if (entity is File) {
        ...
        for (final entry in (config.replacements ?? AssetPathConfig.defaultReplacements).entries) {
  ...
  Future<void> onGenerate(MacroState state) async {
    ...
    buff.writeln('// ignore_for_file: constant_identifier_names, non_constant_identifier_names, library_private_types_in_public_api');
    buff.writeln('// Generated by AssetPathMacro: DO NOT MODIFY BY HAND\n');

    final className = config.generatedClassName?.isNotEmpty == true ? config.generatedClassName : 'AssetPaths';
    ...
    for (final folderName in rootSubfolders) {
      ...
      buff.writeln('  static const $folderName = $className._();');
    }
    ...
    final generatedFileName = config.generatedFileName?.isNotEmpty == true ? config.generatedFileName! : 'assets.dart';
    final file = File(p.join(state.assetState!.absoluteBaseOutputPath, generatedFileName));
  ...
  String _getAssetPath(String filePath, String absoluteBasePath, String relativeBasePath) {
    ...
    return p.posix.join(relativeBasePath, relativeToBase);
  }
  ...
  void _generateFolderClasses(
    ...
    for (final folderPath in sortedFolders) {
      ...
      for (final (fieldName, filePath) in files) {
        final assetPath = _getAssetPath(filePath, absoluteBasePath, relativeBasePath);
        buff.writeln('  static const _${fieldName}Val = \'$assetPath\';\n');
        buff.writeln('  String get $fieldName => _${fieldName}Val;');
      }
      ...
      for (final subfolderName in currentSubfolders) {
        ...
        buff.writeln('  static const _${subfolderName}Val = $subfolderClassName._();\n');
        buff.writeln('  $subfolderClassName get $subfolderName => _${subfolderName}Val;\n');
      }
...
class AssetPathConfig {
  ...
  static AssetPathConfig fromJson(Map<String, dynamic> json) {
    return AssetPathConfig(
      ...
      rename: MacroExt.decodeNullableEnum(FieldRename.values, json['rename'] as String?),
  ...
  ///
  /// Default to `AssetPaths`
  final String? generatedClassName;

  /// The name of generated file
  ///
  /// Default to `assets.dart`
  ...
  ///
  /// Default is [FieldRename.camelCase]
  ...
  @override
  String toString() {
    return 'AssetPathConfig{generatedClassName: $generatedClassName, generatedFileName: $generatedFileName, syncList: $syncList, recursive: $recursive, extension: $extension, rename: $rename, replacements: $replacements}';
  }
}
